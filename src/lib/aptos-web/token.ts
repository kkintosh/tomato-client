...
export type AptosWallet = {
  activeAccount: { address: string; publicKey: string };
  connect: () => void;
  disconnect: () => void;
  connected: boolean;
  network: NetworkInfo;
...
export default class AptosConfig extends BaseWebToken {
  declare protected providerInstance?: Aptos;
  ...
  async getPublicKey(): Promise<string | Buffer> {
    return (this._publicKey ??= this.signingFn
      ...
      : Buffer.from(this.wallet.activeAccount.publicKey.toString().slice(2), "hex"));
  }
